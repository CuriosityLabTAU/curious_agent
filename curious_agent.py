...
class CuriousAgent:
    def __init__(self, index):
        self.q_function = NeuralNetwork(AGENT_Q_NETWORK, linear_relu, min=-1, max=1)
        # input -> state
        # output -> value(state)
        ...
        self.index = index
        # index of the agent in the environment's agents array
    ...
    def reset_network(self):
        if IS_AGENT_RECURRENT:
            ...
            self.learner = NeuralNetwork(AGENT_LEARNER_NETWORK_SHAPE, linear_relu, min=-0.01, max=0.01)
        # input -> [state,action]
        # output -> next state
        self.learner_alpha = AGENT_LEARNER_ALPHA
